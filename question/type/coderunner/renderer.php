...
class qtype_coderunner_renderer extends qtype_renderer {
	...
	if (empty($USER->coderunnerquestionids)) {
        ...
        $indentoption = html_writer::select( array('defaults.cfg'=>'기본', 'gnu-indent.cfg'=>'GNU' , 'linux-indent'=>'LINUX', 'kr-indent.cfg'=>"K.R"),
         '기본', 'defaults.cfg',false);

         $qtext .= $answerprompt . $penaltystring ." ". $indentoption;
    ...
    protected function specific_feedback(question_attempt $qa) {
        ...
        if (!$outcome->has_syntax_error() && !$isprecheck &&
            ...
            $fb .= $this->build_feedback_summary($qa, $outcome, false);
        }
    ...
    public function StartAnalysis($projectKey,$codeType)
    {
        if($codeType == "C" || $codeType == "cpp")
            $this->CheckC($codeType);
        if($codeType == "Java")
            $this->CheckJava($projectKey);
        if($codeType == "python3")
            $this->CheckPython($projectKey);
    }

    public function AnalysisFinished()
    {
        // echo "<script>alert('AnalysisFinished');</script>";
        // $fp = fopen("result.txt", "r");
        // $fr = fread($fp, filesize("result.txt")); 
        // fclose($fp); 
        // echo "<script>alert('{$fr}');</script>";
        // return $fr;
    }

    public function GetDataWithFile($path)
    {
        $fp = fopen($path, "r");
        $data = "";
    ...
    //Use CppCheck
    public function CheckC($codeType)
    {
        if($codeType == "C")
            exec ("php_c.sh");
        else 
        ...
        fwrite ($fw, $this->GetDataWithFile("temp"));
        fclose($fw);
        $this->AnalysisFinished();
    }

    public function CheckJava($projectKey)
    {
        $this->SetSornaQube($projectKey);
        ...
        exec("php_java.exe");
        //exec('curl -u admin:admin -X POST "http://localhost:9000/api/project_analyses/create_event?analysis='.$token.'&name='.$projectKey.'"');
        //$this->GetIssues($projectKey);
    }

    public function CheckPython($projectKey)
    {
        $this->SetSornaQube($projectKey);
        ...
        if(file_exists($codeFile)) {
            if(!copy($codeFile, $destinationFile))
                echo "";
                //echo "<script>alert('파일 복사 실패');</script>";
            else if(file_exists($codeFile))
                echo "";
                //echo "<script>alert('파일 복사 성공');</script>";
        }
    ...
    public function GetIssues($projectKey)
    {
        ...
        $fw = fopen("result.txt", "w");
        fwrite ($fw, $this->GetDataWithJson($output[0]));
        fclose($fw);
    ...
    public function StartChecking()
    {
        ...
        while(false !== ($fileName = readdir($openedDir)))
            if ($fileName != "." && $fileName != "..")
                $fileNames .= $dir."\\".$fileName." ";
        $url = exec ("\Strawberry\perl\bin\perl.exe moss.pl ".$fileNames);
        $content = "'C:\Users\Sieun Kim\AppData\Local\Programs\Python\Python38-32\Lib\site-packages\mossum\mossum.py' -p 80 ".$url;
        $fw = fopen("php_mossum.sh", "w");
        fwrite ($fw, $content);
        fclose($fw);
    ...
    //uncrustify를 사용해 indent 수정 전 후 비교해 바뀐 line#을 저장한 배열을 전달
    protected function get_indent_error_lines($currentanswer, $lang){
        $lineArr = array();  
        $tmp_file = "SRC_SUBMIT.".$lang;  
        $out_file = "SRC_RESULT.".$lang;
        file_put_contents($tmp_file, $currentanswer);
        
        $content = "'C:\Program Files\uncrustify\uncrustify.exe' -c 'C:\Program Files\uncrustify\cfg\gnu-indent.cfg' -f ".$tmp_file." -o ".$out_file;
        $fw = fopen("php_uncrustify.sh", "w");
        fwrite ($fw, $content);
        fclose($fw);
        exec("php_uncrustify.sh");

        $content = "'C:\Program Files (x86)\GnuWin32\bin\diff.exe' -f ".$tmp_file." ".$out_file." > DIFF.diff";
        $fw = fopen("php_diff.sh", "w");
        fwrite ($fw, $content);
        fclose($fw);
        exec("php_diff.sh");
        $fp = fopen("DIFF.diff", "r") or die("파일을 열 수 없습니다！");
      
        while( !feof($fp) ) {
            $getline=fgets($fp, 1000);  
            if($getline[0]==c){
                $substr = substr($getline,1, strlen($getline)); 
                $explodeArr = explode(' ', $substr);
                $i;
                for($i=0; $i<count($explodeArr); $i++){
                    $strToInt =(int)($explodeArr[$i]);
                    if ($strToInt == 0) {
                        break;
                    }
                    else{
                        $explodeArr[$i] = $strToInt;
                    }
                }
                if($i == count($explodeArr))
                    $lineArr = array_merge($lineArr, $explodeArr);
            }
          }
        fclose($fp);
    ...
    protected function build_feedback_summary(question_attempt $qa, qtype_coderunner_testing_outcome $outcome) {
        ...
        if ($outcome->all_correct()) {
            ...
            $lan = $question->language;
            $this->StartAnalysis("201924506", $lan);

            // mkdir("QuizCodes");
            // $fw = fopen("./QuizCodes/".$USER->username, "w");
            // fwrite ($fw, $answer);
            // fclose($fw);
            
            // $this->StartChecking();
            //echo "<script>alert('$QUIZ->id');</script>";
            //echo "<script>alert('$USER->username');</script>";
            ...
            if (!$isprecheck) {
                ...
                fclose($fp);
                if($fr != "")
                    $lines[] = "Code Analysis Result<br>".$fr;
                        
                $currentanswer = $qa->get_last_qt_var('answer');
                $lang = $question->get_language();
                
                if($lang !== "python3")
                {
                    $indentErrorLines =  $this->get_indent_error_lines($currentanswer, $lang);   
                    //  print_r($indentErrorLines);
                    // 결과 메시지 + 인덴트 에러
                    for($i=0; $i<count($indentErrorLines); $i++){
                        $lines[] .= nl2br("(Warning) Line".$indentErrorLines[$i]." : Indent Error");
                    }
                }
            }
        } else {
